...
local function ShootGrappleAtClosestEnemy(slot, data)
    ...
    local cordEnd = Isaac.Spawn(EntityType.ENTITY_EFFECT, LostItemsPack.Entities.LUCKY_SEVEN_CORD_END.variant, 0, slot.Position, targetClawVelocity, slot):ToEffect()
    cordEnd.Parent = slot
    cordEnd:Update()
...
local function SpawnCord(cordEnd)
    local head = Isaac.Spawn(EntityType.ENTITY_EFFECT, LostItemsPack.Entities.LUCKY_SEVEN_CORD_HANDLER.variant, 0, cordEnd.Position, Vector.Zero, cordEnd):ToEffect()
    ...
    local rope = Isaac.Spawn(EntityType.ENTITY_EVIS, 10, LostItemsPack.Entities.LUCKY_SEVEN_CRANE_CORD.subtype, cordEnd.Parent.Position, Vector.Zero, cordEnd)
    cordEnd.Child = rope
    head.Child = rope
...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, OnCordEndUpdate, LostItemsPack.Entities.LUCKY_SEVEN_CORD_END.variant)
...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, OnCordHandlerUpdate, LostItemsPack.Entities.LUCKY_SEVEN_CORD_HANDLER.variant)
...
LostItemsPack:AddCallback(ModCallbacks.MC_PRE_NPC_UPDATE, PreHorfHandlerUpdate, EntityType.ENTITY_HORF)


local function PreEvisCordUpdate(_, npc)
    if npc.Variant == 10 and npc.SubType == LostItemsPack.Entities.LUCKY_SEVEN_CRANE_CORD.subtype then
...
LostItemsPack:AddCallback(ModCallbacks.MC_PRE_NPC_UPDATE, PreEvisCordUpdate, EntityType.ENTITY_EVIS)
