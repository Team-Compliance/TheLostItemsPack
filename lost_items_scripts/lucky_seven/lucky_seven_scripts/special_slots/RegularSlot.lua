...
local function OnCoinTearUpdate(_, tear)
    ...
    tear.SpawnerVariant == LostItemsPack.Entities.LUCKY_SEVEN_SLOT.variant then
        SFXManager():Stop(SoundEffect.SOUND_TEARS_FIRE)
    end
end
LostItemsPack:AddCallback(ModCallbacks.MC_POST_TEAR_UPDATE, OnCoinTearUpdate, TearVariant.COIN)
