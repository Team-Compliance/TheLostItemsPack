...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_RENDER, HUDSparks.OnRender)
...
local function NewSpark(prevPosition, pickup)
    ...
    if Helpers.AnyPlayerHasItem(CollectibleType.COLLECTIBLE_DEEP_POCKETS) and pickup == "Coins" then
...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_UPDATE, HUDSparks.OnFrameUpdate)
...
function HUDSparks:OnShader(shaderName)
    if shaderName ~= "LostItemsPackNothingShader" then return end

    if not Game():GetHUD():IsVisible() then return end
    if not Helpers.AnyPlayerHasItem(LostItemsPack.CollectibleType.LUCKY_SEVEN) then return end
...
LostItemsPack:AddCallback(ModCallbacks.MC_GET_SHADER_PARAMS, HUDSparks.OnShader)