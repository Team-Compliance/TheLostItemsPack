...
function DSSMenuManager.GetSaveData()
    if not LostItemsPack.RunPersistentData.DssMenuConfig then
        LostItemsPack.RunPersistentData.DssMenuConfig = {}
    end

    return LostItemsPack.RunPersistentData.DssMenuConfig
end

function DSSMenuManager.StoreSaveData()
    LostItemsPack:OnGameExit()
end
...
local DSSModName = "Dead Sea Scrolls (Lost Items Pack)"
...
local DSSInitializerFunction = require("lost_items_scripts.lib.dssmenucore")
