...
local function EnemyHpDown(_, enemy)
    local hpEnemy
    for _, hpUpDownEnemy in ipairs(LostItemsPack.PersistentData.HPUpDownEnemies) do
...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_NPC_INIT, EnemyHpDown)
    ...
    for _,enemy in ipairs(Helpers.GetEnemies(false)) do
        ...
        if hpEnemy then
           hpEnemy.Count = hpEnemy.Count - mult
            ...
            table.insert(LostItemsPack.PersistentData.HPUpDownEnemies, hpEnemy)
        end
    end
end)