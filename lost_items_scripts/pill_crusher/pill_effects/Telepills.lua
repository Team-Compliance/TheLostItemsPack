...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_NEW_LEVEL, NewTeleLevel)


local function NewTeleRoom()
	if #LostItemsPack.PersistentData.MonsterTeleTable <= 0 then return end

	local roomIDX = Game():GetLevel():GetCurrentRoomDesc().ListIndex
	local room = Game():GetRoom()

	for index, teleMonster in ipairs(LostItemsPack.PersistentData.MonsterTeleTable) do
		if teleMonster.RoomIDX == roomIDX then
			...
			if teleMonster.SpawnPos == nil then
				LostItemsPack.PersistentData.MonsterTeleTable[index].SpawnPos = spawnpos
			else
...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, NewTeleRoom)
...
local function CleanRoom()
	...
	for _, teleMonster in ipairs(LostItemsPack.PersistentData.MonsterTeleTable) do
		if teleMonster.RoomIDX ~= currentRoomIndex then
			newMonsterTeleTable[#newMonsterTeleTable+1] = teleMonster
		end
	end

	LostItemsPack.PersistentData.MonsterTeleTable = newMonsterTeleTable
end
...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_UPDATE, OnUpdate)
	...
	if data.TeleFrames > #TeleportAnimFrames*2 then
		if data.IsTeleportingBack then
			...
			if not data.WasHorseTelePilled then
				...
				table.insert(LostItemsPack.PersistentData.MonsterTeleTable, {
...
LostItemsPack:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, TeleportMonsterAnim)
